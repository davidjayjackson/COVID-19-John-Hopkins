...
ECDC_dat <- JHUDATA<- dbGetQuery(db, "SELECT countries,Reported,Cases,Deaths,Recovered FROM JHUDATA;")
...
```{r,echo=FALSE}   
# ECDC_Countries_LT50 <- ECDC_Countries %>% filter(Cases <= 49)
# ECDC_Countries_50 <- ECDC_Countries %>% filter(Cases >=1000)
# ECDC_Countries_500 <- ECDC_Countries %>% filter(Cases >=2000)
# ECDC_Countries_1000 <- ECDC_Countries %>% filter(Cases >=4000)
# ECDC_Countries_5000 <- ECDC_Countries %>% filter(Cases >=8000)
# ECDC_Countries_10000 <- ECDC_Countries %>% filter(Cases >=16000)
```

#### Total Confimed  Cases by Countries

```{r,echo=FALSE}
ECDC %>% filter(Cases <= 250) %>%
...
```{r, echo=FALSE}
 ECDC %>% ggplot() + geom_col(aes(x=reorder(Countries,DeathRate),y=DeathRate)) +
...
ggplot() + geom_line(data=US_total_daily,aes(x=Reported,y=Cases,col="US")) +
  geom_line(data=world_total_daily,aes(x=Reported,y=Cases,col="World")) +
...
world_total_daily$E <- movavg(world_total_daily$Cases, 20,"e")
world_total_daily %>% ggplot() +geom_line(aes(x=Reported,y=Cases,col="Cases")) +
...
dbWriteTable(db, "JHULASTEST",ECDC ,overwrite=TRUE)    
